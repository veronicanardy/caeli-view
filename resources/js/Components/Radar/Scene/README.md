...
- `KnownSpacecraftLayer.tsx`: naves famosas (Voyager 1/2, Pioneer 10/11, New Horizons, Juno, James Webb, Parker Solar Probe, Europa Clipper) com o GLB oficial da NASA (`Bodies/Spacecraft/SpacecraftModel`; o `SpacecraftMarker` estilizado fica como fallback do Suspense). Diferente das camadas acima, é montada SEMPRE (como os planetas, fora do modo famosos) e não recebe `skipIds`: as naves não estão no feed, então o `AsteroidMarker` as pintaria como rocha. A posição vem de `knownSpacecraft` (Horizons ao vivo com fallback local, não Kepler); a prop `earthHelioAU` habilita o caminho preciso `Terra_exata + geoAU` e o fallback L2 do James Webb. O clique chama `onFocusSpacecraft`, que foca a câmera e abre o card sintético da nave.
...
- `sceneWarmupGpu.ts`: primitivas SEM React do aquecimento da GPU (`collectPendingTextures` varre a cena por texturas ainda não enviadas; `warmupSceneOnce` compila os materiais via `compileAsync` e sobe as texturas pendentes de uma vez). Reutilizado pelo `SceneWarmup` (passes ociosos) e pelo gate do primeiro frame em `RadarScene` (aquecimento atrás da barra de carregamento). `collectPendingTextures` é testado em `tests/js/Radar/Scene/sceneWarmupGpu.test.ts`.
- `SceneWarmup.tsx`: pré-compilação assíncrona de shaders e upload escalonado de texturas em momentos ociosos (passes em intervalos crescentes), para que revelar objetos novos ao rotacionar a câmera não dispare compilação síncrona no meio do gesto. Usa as primitivas de `sceneWarmupGpu.ts`. Complementa o aquecimento inicial: o gate do primeiro frame (`FirstFrameNotifier`, neste arquivo) já aquece a GPU UMA vez atrás da barra de carregamento, antes de liberar a cena, pagando o custo de upload/compilação escondido em vez de no primeiro gesto após entrar; o `SceneWarmup` segue cobrindo o que chega depois (efeméride, GLBs com preload adiado).

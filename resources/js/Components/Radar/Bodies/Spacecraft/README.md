...
* **Não têm diâmetro nem aproximação.** São estruturas de poucos metros, sub-pixel em qualquer escala. O card mostra os campos físicos como null (como os cometas sem aproximação) e o marcador é estilizado, não escalado por tamanho real.
...
* `SpacecraftMarker.tsx`: forma 3D SIMBÓLICA e sóbria de sonda (corpo central, dois painéis solares, antena de prato), sem GLB. Tom metálico frio e discreto, com leve emissivo só na seleção/hover. É o equivalente ao "genérico" dos cometas; um modelo real por nave pode vir depois (padrão da skill: genérico agora, modelo real depois).

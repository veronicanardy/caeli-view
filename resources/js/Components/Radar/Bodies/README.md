...
* `Comet`: identidade fixa e elementos orbitais dos cometas famosos (`knownComets.ts`), usados como posição de fallback Kepler na cena. Contraparte cometária de `Asteroid/knownAsteroids.ts`. Enquanto não há GLB exclusivo de cometa, eles reusam o modelo genérico de asteroide recolorido com um tint gelado (ver `Scene/KnownCometsLayer`).

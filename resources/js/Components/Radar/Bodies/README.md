...
* `Spacecraft`: identidade e posição das naves famosas (Voyager 1/2, Pioneer 10/11, New Horizons, Juno, James Webb, Parker Solar Probe, Europa Clipper), com o GLB oficial da NASA e marcador estilizado de fallback (`knownSpacecraft.ts`, `SpacecraftModel.tsx`, `SpacecraftMarker.tsx`). Diferente de asteroides/cometas, as naves vivem na cena como os planetas (sempre presentes, fora do feed) e usam posição do Horizons ao vivo com fallback local, não Kepler (estão em escape hiperbólico, orbitando um planeta ou acompanhando a Terra no L2). Ver `Spacecraft/README.md`. Satélites de órbita terrestre ficam de fora (não cabem na régua heliocêntrica); o James Webb entra porque o ponto L2 fica bem fora do disco visual da Terra.

...
- `ReferenceControls.tsx`: atalhos de foco para Sol, Terra, Lua, planetas e naves, exibidos com símbolos astronômicos Unicode (☉ ♁ ☽ ✦ ⦿). Os controles ficam em duas faixas separadas por natureza: a primeira linha foca a câmera direto num corpo (Sol/Terra/Lua), a segunda abre grupos que expandem flyout (Planetas/Naves). A divisão evita o aperto antigo, em que os cinco controles dividiam uma linha só e o texto longo encostava nos divisores. O wrapper das duas faixas carrega `data-tutorial="reference-controls"` para o passo de referências do tutorial. A prop `labelsAlwaysVisible` força os rótulos de texto abaixo de sm: (uso nos sheets mobile, onde há largura). Além do `PlanetFlyout`, expõe o `SpacecraftFlyout`: lista das naves famosas (Voyager 1/2, Pioneer 10/11, New Horizons, Juno, James Webb, Parker Solar Probe, Europa Clipper), cada item foca a câmera na nave e abre o card dela. O botão "Naves" e o de "Planetas" são mutuamente exclusivos (abrir um fecha o outro), controlado pelo container via `spacecraftOpen`/`planetsOpen`. Durante o tutorial todos os cinco controles respeitam o gate de ações: cada um consulta `isActionAllowed` (`focus-sun`/`focus-body`/`toggle-planets`/`toggle-spacecraft`) e fica `disabled` quando a ação não é permitida no passo, então o botão Naves apaga junto dos outros em vez de continuar aceso.

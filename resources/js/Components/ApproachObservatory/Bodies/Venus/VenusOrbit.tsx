/**
 * VenusOrbit — anel de referência geocêntrico de Vênus.
 *
 * Mesma abordagem de MercuryOrbit: anel circular no plano eclíptico (XZ da cena)
 * com raio igual à distância geocêntrica atual de Vênus.
 *
 * Cor levemente âmbar/dourada para distinguir do anel prateado de Mercúrio
 * e do anel azul da Terra.
 */

import { useMemo } from 'react';
import * as THREE from 'three';

const ORBIT_SEGMENTS = 128;
const ORBIT_COLOR = '#c8b880';   // dourado suave — remete à atmosfera âmbar de Vênus
const ORBIT_OPACITY = 0.22;

interface VenusOrbitProps {
    /** Venus position in scene units — the ring radius is derived from this. */
    venusPos: [number, number, number];
}

export function VenusOrbit({ venusPos }: VenusOrbitProps) {
    const orbitPoints = useMemo(() => {
        const radius = Math.hypot(venusPos[0], venusPos[1], venusPos[2]);
        if (radius < 1e-6) return null;

        const pts = new Float32Array((ORBIT_SEGMENTS + 1) * 3);
        for (let i = 0; i <= ORBIT_SEGMENTS; i += 1) {
            const a = (i / ORBIT_SEGMENTS) * Math.PI * 2;
            pts[i * 3]     = Math.cos(a) * radius;
            pts[i * 3 + 1] = 0;
            pts[i * 3 + 2] = Math.sin(a) * radius;
        }
        return pts;
    }, [venusPos]);

    if (!orbitPoints) return null;

    return (
        <line>
            <bufferGeometry attach="geometry">
                <bufferAttribute attach="attributes-position" args={[orbitPoints, 3]} />
            </bufferGeometry>
            <lineBasicMaterial
                color={ORBIT_COLOR}
                transparent
                opacity={ORBIT_OPACITY}
                depthWrite={false}
            />
        </line>
    );
}

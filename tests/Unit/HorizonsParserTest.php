...
/**
 * Covers the Horizons text parsers (vector table + osculating-elements header) in isolation.
 */
class HorizonsParserTest extends TestCase
{
    private function parser(): HorizonsTextParser
    {
        return $this->app->make(HorizonsTextParser::class);
    }

    /**
     * @return array<int, array<string, float|string|null>>
     */
    private function pointArrays(string $text): array
    {
        return array_map(
            static fn (HorizonsVectorPointData $point): array => $point->toArray(),
            $this->parser()->parseVectorPoints($text),
        );
    }

    public function test_has_ephemeris_detects_both_sentinels(): void
    {
        $this->assertTrue($this->parser()->hasEphemeris(JplResponses::horizonsVectorsText()));
    }

    public function test_has_ephemeris_returns_false_when_either_sentinel_is_missing(): void
    {
        $this->assertFalse($this->parser()->hasEphemeris('$$SOE only, no closing sentinel'));
        $this->assertFalse($this->parser()->hasEphemeris('no opening sentinel $$EOE'));
        $this->assertFalse($this->parser()->hasEphemeris(''));
    }

    public function test_parse_vector_points_extracts_position_velocity_and_range(): void
    {
        $points = $this->pointArrays(JplResponses::horizonsVectorsText());
...
TEXT;
    ...
    {
        $elements = $this->parser()->parseOrbitalElements(JplResponses::horizonsVectorsWithElementsText())?->toArray();
...
$$EOE
TEXT;
        $this->assertNull($this->parser()->parseOrbitalElements($headerMissingShape));
    }
...
$$EOE
TEXT;
        $elements = $this->parser()->parseOrbitalElements($minimal)?->toArray();
...
$$EOE
TEXT;
        $elements = $this->parser()->parseOrbitalElements($compact)?->toArray();
